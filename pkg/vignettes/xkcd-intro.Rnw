...
%% Time-stamp: <2013-05-04 Sat 23:48 emilio on emilio-laptop2>
%% =====================================================================
...
First, check if the XKCD fonts are installed in the computer.
...
If the XKCD fonts, or other fonts, are not installed in the system, you may read the tutorial 
...
It uses the package \texttt{extrafont}, with detailed instructions in \url{https://github.com/wch/extrafont}: 
\begin{itemize}
    \item Option I:
<<eval=FALSE>>=
## Borrowed from
...
If you want to uninstall the fonts, an easy way is to remove the following packages:
<<eval=FALSE>>=
...
\begin{center}
<<fig=TRUE, png=TRUE, pdf=FALSE, eps=FALSE, center = TRUE>>=
...
timelabel <-  function(text,x,y) {
  list(annotate("text", x=x, y = y + 65, label=text, size = 6,family ="xkcd"),
...
volun <- c("Pilar","Maria","Miguel","Jose","Juan",
           "Rocio","Tata","Violeta","Titi","Jandro","Dani")
