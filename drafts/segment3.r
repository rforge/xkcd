...
# Time-stamp: <2013-04-30 Tue 23:36 emilio on emilio-laptop2>
# =====================================================================


### BORRADOR MIRA EL SEGMENT 2
...
segment <- function(mapping, data, ...) {
  ...
  ## Are xjitter, yjitter npoint present in this call?
  ## If yes, we will expand to build a vector of the same length that the data
  ## I know, this a very, very dirty trick
  argList<-list(...)
 fcn <- get("interpolate", mode = "function")
  argsofinterpolate <-  names(formals(fcn))
  ##listofargumentstoreplicate <- c("xjitteramount","yjitteramount","npoints")
  ##for( arg in intersect(listofargumentstoreplicate, names(argList))) {
  for( arg in intersect(argsofinterpolate, names(argList))) {
    if(!(is.null(argList[arg])==TRUE)){
      if(length(argList[[arg]]) == 1 ) argList[[arg]] <- unlist(rep(argList[[arg]],nsegments))
      if(length(argList[[arg]]) != nsegments )
        stop(paste("argument", arg," is not a vector of length",nsegments))
     }
   }

  
  listofpaths <- lapply(1:nsegments,
                        function(i, data, namesofdataaesthetics, othervariablenames, argList) {
                          ## Check the arguments that are in the interpolate function
                          ## For that arguments, modify them to get only a value
                          fcn <- get("interpolate", mode = "function")
                          argsofinterpolate <-  names(formals(fcn))
                          for(arg in names(argList)) {
                            if( arg %in% argsofinterpolate) {
                              if(length(argList[[arg]]) == dim(data)[1] ){
                                argList[[arg]] <- argList[[arg]][i]
                              }
                            }
                          }
                          
                          if(length(argList))
                             newdata <- do.call("interpolate",
                                             c(
                                               xbegin = data[i, namesofdataaesthetics[1]],
                                                 ybegin = data[i, namesofdataaesthetics[2]],
                                                 xend = data[i, namesofdataaesthetics[3]],
                                                 yend = data[i, namesofdataaesthetics[4]],
                                                  argList)
                                                 )
                         else newdata <- do.call("interpolate",
                                             list(
                                             xbegin = data[i, namesofdataaesthetics[1]],
                                                 ybegin = data[i, namesofdataaesthetics[2]],
                                                 xend = data[i, namesofdataaesthetics[3]],
