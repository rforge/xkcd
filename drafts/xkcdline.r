...
# Time-stamp: <2013-05-01 Wed 13:49 emilio on emilio-laptop2>
# =====================================================================
...
xkcdline <- function(mapping, data,typexkcdline=c("segment","circle"), mask = TRUE, ...) {

  if(typexkcdline[1] == "segment" ){
    fun <- "interpolate"
...
kk <- xkcdline(mapping=aes(x=x1, y=y1, xend=xend1, yend=xend1) , data = data, size=4,mask=TRUE)
dibujobase +kk

dibujobase + xkcdline(mapping=aes(x=x1,y=y1,diameter=x1), data=data, typexkcdline="circle" )
